...
void main() {
	...
	// Glitch based on the work of Ian McEwan, Ashima Arts, https://github.com/ashima/webgl-noise
	if(uGlitch > 0) {
		...
		float y = pixelCoords.y * resolution.y;
		float rgbWave = (
		...
		float whiteNoise = (random(pixelCoords + mod(uTime, 10.0)) * 2.0 - 1.0) * (0.15 + strength * 0.15);
		if(pixelCoords.y % int(uGlitch/rgbWave) < rgbWave*10 && pixelCoords.x % int(uGlitch*rgbWave) > rgbWave*10) {
			float bnTime = floor(uTime * 20.0) * 200.0;
			float noiseX = step((snoise3(vec3(0.0, pixelCoords.x * 3.0, bnTime)) + 1.0) / 2.0, 0.12 + strength * 0.3);
			float noiseY = step((snoise3(vec3(0.0, pixelCoords.y * 3.0, bnTime)) + 1.0) / 2.0, 0.12 + strength * 0.3);
			float bnMask = noiseX * noiseY;
	          
			float bnTime2 = floor(uTime * 25.0) * 300.0;
			float noiseX2 = step((snoise3(vec3(0.0, pixelCoords.x * 2.0, bnTime2)) + 1.0) / 2.0, 0.12 + strength * 0.5);
			float noiseY2 = step((snoise3(vec3(0.0, pixelCoords.y * 8.0, bnTime2)) + 1.0) / 2.0, 0.12 + strength * 0.3);
