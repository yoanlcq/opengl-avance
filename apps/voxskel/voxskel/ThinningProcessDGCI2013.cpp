...
void ThinningProcessDGCI2013::updateBorderXNEG(VoxelList& updatedBorder) {
   for (auto i = 0u; i < m_Borders[0].size(); i++){
        ...
        if (!isInBorder(p[0], p[1], p[2])){
            updatedBorder.emplace_back(p);
            setBorderFlag(p[0], p[1], p[2]);
        }

        // Testing neighbourhood
        if (p[0] > 0 && !isInBorder(p[0] - 1, p[1], p[2])){
            updatedBorder.emplace_back(Vec3i{ p[0] - 1, p[1], p[2] });
            setBorderFlag(p[0] - 1, p[1], p[2]);
        }

        if (p[1] > 0 && !isInBorder(p[0], p[1] - 1, p[2])){
            updatedBorder.emplace_back(Vec3i{ p[0], p[1] - 1, p[2] });
            setBorderFlag(p[0], p[1] - 1, p[2]);
        }

        if (p[2] > 0 && !isInBorder(p[0], p[1], p[2] - 1)){
            updatedBorder.emplace_back(Vec3i{ p[0], p[1], p[2] - 1 });
            setBorderFlag(p[0], p[1], p[2] - 1);
        }

        if (p[1] < int(m_nHeight) - 1 && !isInBorder(p[0], p[1] + 1, p[2])){
            updatedBorder.emplace_back(Vec3i{ p[0], p[1] + 1, p[2] });
            setBorderFlag(p[0], p[1] + 1, p[2]);
        }

        if (p[2] < int(m_nDepth) - 1 && !isInBorder(p[0], p[1], p[2] + 1)){
            updatedBorder.emplace_back(Vec3i{ p[0], p[1], p[2] + 1 });
            setBorderFlag(p[0], p[1], p[2] + 1);
        }
    }
...
void ThinningProcessDGCI2013::updateBorderXPOS(VoxelList& updatedBorder) {
   for (auto i = 0u; i < m_Borders[1].size(); i++){
        ...
        // Testing neighbourhood
        if (p[0] < int(m_nWidth) - 1 && !isInBorder(p[0] + 1, p[1], p[2])){
            updatedBorder.emplace_back(Vec3i{ p[0] + 1, p[1], p[2] });
            setBorderFlag(p[0] + 1, p[1], p[2]);
        }
...
void ThinningProcessDGCI2013::clearBorderFlags(const VoxelList& updatedBorder) {
    for (auto i = 0u; i < updatedBorder.size(); ++i) {
        Vec3i p = updatedBorder[i];
        setBorderFlag(p[0], p[1], p[2], false);
    }
}
