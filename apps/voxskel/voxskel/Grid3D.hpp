...
public:
    ...
    Grid3D(const Vec3u& resolution): Grid3D(resolution[0], resolution[1], resolution[2]) {
    }

    Grid3D(const Vec3u &resolution, T value): Grid3D(resolution[0], resolution[1], resolution[2], value) {
    }
    ...
    uint32_t offset(const Vec3i& coords) const {
        return offset(coords[0], coords[1], coords[2]);
    }

    Vec3i coords(uint32_t offset) const {
        Vec3i c;
        c[0] = offset % m_nWidth;
        c[1] = ((offset - c[0]) / m_nWidth) % m_nHeight;
        c[2] = ((offset - c[0] - c[1] * m_nWidth)) / m_nSliceSize;
        return c;
    }
    ...
    bool contains(const Vec3i& coords) const {
        return contains(coords[0], coords[1], coords[2]);
    }
    ...
    Vec3u resolution() const {
        return Vec3u{ uint32_t(m_nWidth), uint32_t(m_nHeight), uint32_t(m_nDepth) };
    }
...
public:
    ...
    Grid3D(const Vec3u &resolution, bool value): Grid3D(resolution[0], resolution[1], resolution[2], value) {
    }
...
void processXCut(const Vec3u& gridSize, uint32_t cutPosition, Functor f) {
    for(auto z = 0u; z < gridSize[2]; ++z) {
        for(auto y = 0u; y < gridSize[1]; ++y) {
...
inline Vec2u getCutSize(const Vec3u& gridSize, Grid3DCutAxis cutAxis) {
    switch(cutAxis) {
    case Grid3DCutAxis::XAxis:
        return Vec2u{ gridSize[1], gridSize[2] };
    case Grid3DCutAxis::YAxis:
        return Vec2u{ gridSize[2], gridSize[0] };
    case Grid3DCutAxis::ZAxis:
        return Vec2u{ gridSize[0], gridSize[1] };
    }
    return { 0, 0 };
}
...
inline void foreach6Neighbour(const Vec3u& gridSize, const Vec3i& voxel, Functor f) {
    if(voxel[0] > 0) {
        f(Vec3i{ voxel[0] - 1, voxel[1], voxel[2] });
    }
    if(voxel[1] > 0) {
        f(Vec3i{ voxel[0], voxel[1] - 1, voxel[2] });
    }
    if(voxel[2] > 0) {
        f(Vec3i{ voxel[0], voxel[1], voxel[2] - 1 });
    }
    if(voxel[0] < int(gridSize[0]) - 1) {
        f(Vec3i{ voxel[0] + 1, voxel[1], voxel[2] });
    }
    if(voxel[1] < int(gridSize[1]) - 1) {
        f(Vec3i{ voxel[0], voxel[1] + 1, voxel[2] });
    }
    if(voxel[2] < int(gridSize[2]) - 1) {
        f(Vec3i{ voxel[0], voxel[1], voxel[2] + 1 });
    }
}
...
inline void foreach26Neighbour(const Vec3u& gridSize, const Vec3i& voxel, Functor f) {
    for(auto k = -1; k <= 1; ++k) {
        for(auto j = -1; j <= 1; ++j) {
            for(auto i = -1; i <= 1; ++i) {
                if(i != 0 || j != 0 || k != 0) {
                    Vec3i neighbour = Vec3i{ voxel[0] + i, voxel[1] + j, voxel[2] + k };
                    if(neighbour[0] >= 0 && neighbour[1] >= 0 && neighbour[2] >= 0 &&
                            neighbour[0] < int(gridSize[0]) && neighbour[1] < int(gridSize[1]) && neighbour[2] < int(gridSize[2])) {
                        f(neighbour);
                    }
                }
            }
...
inline void foreachVoxel(const Vec3u& gridSize, Functor f) {
    for (auto z = 0u; z < gridSize[2]; ++z) {
        for (auto y = 0u; y < gridSize[1]; ++y) {
            for (auto x = 0u; x < gridSize[0]; ++x) {
                f(Vec3i{ int32_t(x), int32_t(y), int32_t(z) });
            }
        }
    }
}
