...
bool CubicalComplex3D::pierceSurfacicParts() {
    Grid3D<bool> visitMap(resolution(), false);

    std::vector<Vec3i> surfacicVoxels;

    foreachVoxel(resolution(), [&](const Vec3i& voxel) {
        if((*this)(voxel).containsSome(CC3DFaceBits::XYFACE | CC3DFaceBits::XZFACE | CC3DFaceBits::YZFACE) &&
                !visitMap(voxel)) {
            surfacicVoxels.emplace_back(voxel);
            visitMap(voxel) = true;
            ...
            while(!stack.empty()) {
                ...
                foreach6Neighbour(resolution(), top, [&](Vec3i neighbor) {
                    if(!visitMap(neighbor) && (*this)(neighbor).containsSome(CC3DFaceBits::XYFACE | CC3DFaceBits::XZFACE | CC3DFaceBits::YZFACE)) {
                        visitMap(neighbor) = true;
    ...
    for(const auto& voxel: surfacicVoxels) {
        if((*this)(voxel).containsSome(CC3DFaceBits::XZFACE)) {
            (*this)(voxel).remove(CC3DFaceBits::XZFACE);
        } else if((*this)(voxel).containsSome(CC3DFaceBits::XYFACE)) {
            (*this)(voxel).remove(CC3DFaceBits::XYFACE);
        } else {
            (*this)(voxel).remove(CC3DFaceBits::YZFACE);
        }
    }

    return !surfacicVoxels.empty();
}
...
Grid3D<uint32_t> computeCurvilinearSkeletonMappingGrid(const CubicalComplex3D& originalObjectCC,
    ...
    // BASIC ONE (only geodesic propagation)
    Grid3D<uint32_t> mappingGrid(originalObjectCC.resolution(), UNDEFINED_NODE);

    std::queue<CCPoint> mappingQueue;

    // Put all nodes in a queue and fill their grid cell with their index
    for(auto i = 0u; i < nodes.size(); ++i) {
        mappingGrid(std::get<0>(nodes[i])) = i;
        mappingQueue.push(nodes[i]);
    }

    // Perform a breadth first search, starting at node cells, and propagating their index
    // in a region having the same opening (d26) to avoid growing to much the region of influence
    // of nodes located in narrow regions
    while(!mappingQueue.empty()) {
        ...
        foreach6Neighbour(mappingGrid.resolution(), std::get<0>(point), [&](const Vec3i& voxel) {
            // If the voxel belongs to the original object (before thinning) and the cell has not be associated to an
            // index
            if(originalObjectCC(voxel).containsSome(CC3DFaceBits::CUBE) && mappingGrid(voxel) == UNDEFINED_NODE) {
                // Enqueue the new voxel and set its index
    ...
    // WITH RESPECT TO OPENINGMAP (seems bugged, strange association at some points...):
//    Grid3D<uint32_t> mappingGrid(originalObjectCC.resolution(), UNDEFINED_NODE);
...
//            // index
//            if(originalObjectCC(voxel).containsSome(CC3DFaceBits::CUBE) && mappingGrid(voxel) == UNDEFINED_NODE) {
//                // Check the opening constraint
//                if(opening26Map(voxel) == std::get<2>(point)) {
//                    // Enqueue the new voxel and set its index
//                    mappingGrid(voxel) = mappingGrid(std::get<0>(point));
//                    mappingQueue.push(std::make_tuple(voxel, std::get<1>(point), std::get<2>(point)));
//                } else if(!isBorderVoxel) {
//                    // The opening changes, put the voxel has a border voxel of the current region
//                    isBorderVoxel = true;
...
CubicalComplex3D getCubicalComplex(const VoxelGrid& voxelGrid, bool getComplementary) {
    ...
    auto w = voxelGrid.resolution()[0];
    auto h = voxelGrid.resolution()[1];
    auto d = voxelGrid.resolution()[2];
